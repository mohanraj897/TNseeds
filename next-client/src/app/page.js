...
import { Sprout, RefreshCw, ChevronLeft, ChevronRight, Wifi, WifiOff, Leaf } from 'lucide-react'
...
export default function Home() {
  ...
  const { showSeedForm, setShowSeedForm } = useUI()
  ...
  useEffect(() => {
    ...
    socket.on('order:updated', (updated) => {
      if (user && (updated.farmer === user._id || updated.dealer === user._id)) {
  ...
  /* ── GPS locate ── */
  const handleLocate = () => {
    if (typeof window === 'undefined' || !navigator.geolocation) return toast.error('Geolocation not supported')
    setLocating(true)
  ...
  const handleEdit = (seed) => { setEditSeed(seed); setShowSeedForm(true) }
  ...
  const handleCloseForm = () => { setShowSeedForm(false); setEditSeed(null) }
      ...
      <section className="hero">
        <div className="container">
          ...
          <div className="hero-stats">
            <div className="stat"><span>{pagination.total}</span> Seeds Listed</div>
            <div className="stat-sep"/>
            <div className="stat"><span>Real-time</span> Updates</div>
            <div className="stat-sep"/>
            <div className="stat">
              <span className={connected ? 'online' : 'offline'}>
                {connected ? <><Wifi size={14}/> Live</> : <><WifiOff size={14}/> Offline</>}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="container main-content">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            {isDealer && (
              <button className="btn btn-primary" onClick={() => setShowSeedForm(true)}>
