...
async function seed() {
  try {
    ...
    // Create users
    const dealers = await User.create(DEMO_DEALERS)
    const farmer  = await User.create(DEMO_FARMER)
    console.log(`👥 Created ${dealers.length} dealers + 1 farmer`)

    // Create seeds split across dealers
    const seedDocs = SEEDS_DATA.map((s, i) => ({
      ...s,
      isAvailable: true,
    ...
    await Seed.create(seedDocs)
    console.log(`🌱 Created ${seedDocs.length} seed listings`)

    console.log('\n✨ Demo data seeded successfully!')
    console.log('📧 Dealer login: dealer@demo.com / demo123')
    console.log('📧 Farmer login: farmer@demo.com / demo123')
    process.exit(0)
  } catch (e) {
